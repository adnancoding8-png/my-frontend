...
const updateWhatsAppSettings = async (req, res) => {
  try {
    let { number, message, enabled, phoneNumberId, accessToken } = req.body;
    ...
    if (phoneNumberId && phoneNumberId.trim().length === 0) {
      errors.phoneNumberId = "Phone Number ID cannot be empty if provided";
    }
    
    // Validate Access Token if provided
    if (accessToken && accessToken.trim().length === 0) {
      errors.accessToken = "Access Token cannot be empty if provided";
    }
    ...
    if (phoneNumberId !== undefined) {
      updates.push(Settings.setSetting('whatsapp_phone_number_id', phoneNumberId));
    }
    
    if (accessToken !== undefined) {
      updates.push(Settings.setSetting('whatsapp_access_token', accessToken));
    }
